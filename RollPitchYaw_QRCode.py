...
"""
drone_qr_align_full_control_sitl.py
-----------------------------------

Fully functional for ArduPilot SITL (GUIDED mode).
Performs 3D alignment (forward/backward, left/right, up/down, yaw) with a detected QR code.

Fixes:
- Yaw not working (uses MAV_CMD_CONDITION_YAW instead of yaw_rate in velocity command)
- Roll response tuned for SITL

Press 'q' in the camera window to land and exit.
"""
...
def qr_code_alignment(vehicle):
    ...
    while True:
        ...
        if qr_codes:
            ...
            condition_yaw(vehicle, np.degrees(yaw_rate))  # rad/s → deg/s
